...
class Discriminator(tf.keras.layers.Layer):
    def __init__(self, bn=True, stride=2, filters=64):
        ...
        self.model_output = tf.keras.layers.Conv2D(1, 4, 1, padding="valid")

    def make_discriminator_block(self):
        label = []
        label.append(Conv_Downsample_Block(self.filters, self.stride, bn=False))
        for _ in range(2):
            self.filters = self.filters * 2
            label.append(Conv_Downsample_Block(self.filters, self.stride, self.bn))

        self.filters = self.filters * 2
        label.append(
            Conv_Downsample_Block(
                filters=self.filters, stride=1, bn=self.bn, last_layer=True
            )
        )
        return tf.keras.Sequential(label)

    def call(self, image_1, image_2, training=None):
        x = tf.concat([image_1, image_2], axis=-1)
